...
conn.execute "update zh_AdminGroup set sonid='"&sonid&"' where id="&Id


'cateids=split(cateids,",")
rs.open "select cateid from category order by depth,orderid",conn,1,1
cateids=rs.getRows()
rs.close
...
rs.open "Select * from zh_AdminGroupPrivileges where groupid="&id&" and Ptype=1 and cateid="&cateid&" ",conn,1,3
if not rs.eof then
rs("level")=level
rs.update
else
...
response.Redirect "Manage_Admin_permission.asp?id="&id
...
<table width="700" border="0" cellpadding="2" cellspacing="1" class="table_southidc">
        <tr> 
          <td height="25" align="center" class="back_southidc"><strong><%=rs("title")%> 栏目权限设置</td>
        </tr>
        <tr class="tr_southidc"> 
          <form name="form1"  action="Manage_Admin_permission.asp" method="post">
			...
			rs2.open "select cateid,catename,followid,depth,sonid,parentid,orderID from category where catetype<=5 order by depth,orderID",conn,1,1
			if not rs2.eof then
				data= rs2.getRows()
			end if
			rs2.close
...
If data(3,i)>1 then class_list=class_list& "&nbsp;&nbsp;"
		...
		rs.open "select * from 	zh_AdminGroupPrivileges where groupid ="&id&" and ptype=1 and cateid="&data(0,i)
		if not rs.eof then
			plevel=rs("level")
		else
			plevel=0
...
$("#inputlist input[name=sonid]").click(function(){
	...
	for (i=curClassNum-1; i>=1; i--){
		$(this).parent().parent().prevAll("tr.tr_southidc"+i+"").first().children("td").find(".td_southidc"+i+"").prop("checked", true);
                ...
                <tr> 
                  <td height="22" colspan="1"><a id="setall" href="javascript:;">全选</a>   <a id="setNotall" href="javascript:;">不选</a>  <!-- <a id="setallC" href="javascript:;">反选</a>--> 
