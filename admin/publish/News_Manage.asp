...
sql="select publish.*,c.catename from publish left join category c on publish.cateid=c.cateid where isdel=0"
if cateid<>0 then
	sql=sql & " and publish.cateid in (" & sonid & ")"	
end if
if keywords<>"" then
	sql=sql &" and (publish.title like '%"&keywords&"%' or publish.content like '%"&keywords&"%')"
end if
