...
if Action="Add" then
	...
	rs.open "Select * From Catalogs Where title='" & title & "'",conn,1,3
...
ElseIf Action="Del" Then
	Conn.Execute("Delete from Catalogs Where ID="&ID&"")
	Response.Redirect "ad_sort.asp"
ElseIf Action="Update" Then
	...
	End If
	Set rs=Server.CreateObject("Adodb.RecordSet")
	rs.open "Select * From Catalogs Where title='" & title & "' and ID<>"&ID&"",conn,1,3
	if not (rs.bof and rs.EOF) then
		Response.Write("<script language=JavaScript>alert(""此类别已存在!"");location.href=""javascript:history.back(-1);"";</script>")
		Response.End()
	else
     	Conn.Execute("Update Catalogs Set title='"&title&"' Where ID="&ID&"")
    	call CloseConn()
		Response.Redirect "ad_sort.asp"  
	end if
end if
...
rsClass.open "Select * From Catalogs Order By id",conn,1,3
%>
...
%>
  <tr bgcolor="#F2F8FF" class="tdbg">
    ...
    <td align="left" bgcolor="#F2F8FF" class="tr_southidc" style="padding-right:10">&nbsp;<a href="ad_sort.asp?Act=Modi&ID=<%=rsClass("ID")%>">修改</a> | <a href="ad_sort.asp?Act=Del&Id=<%=rsClass("ID")%>" onClick="return ConfirmDelBig();">删除</a> | <a href="ad_add.asp?sortid=<%=rsClass("ID")%>">添加广告</a> | <a href="ad_manage.asp?sortid=<%=rsClass("ID")%>">管理广告</a></td>
  </tr>
<%
rsClass.MoveNext
Loop
...
<%
If Action="Modi" Then
Set Rs=Conn.Execute("select * from Catalogs Where ID="&ID&"")
	if not rs.eof then
		title=rs("title")
		ID=rs("id")
	else
