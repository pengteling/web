...
if request("act")="modi" then
	...
	rs.open "select * from photos where id ="&id,conn,1,3
	
		rs("rid")=request("catalogid")
		rid=request("catalogid")
		rs("title")=title
	...
	response.Redirect "ad_manage.asp?sortid="&rid
	response.End()
end if
...
%>
<FORM method="POST" name="form1" action="ad_mod.asp" style="margin:0 0 0 0;">
...
<TR bgcolor="#F5F5F5">
  ...
  <TD align="left" bgcolor="#ECF5FF"><input name="title" type="text" id="title" size="60" value="<%=title%>">
...
<TR bgcolor="#F5F5F5">
    ...
    <TD align=left bgcolor="#ECF5FF">
        ...
        <input type=button onClick="window.location.href='ad_sort.asp'" name=b2 value="返 回">       
    </TD>
</TR>
</TABLE>
</FORM>
