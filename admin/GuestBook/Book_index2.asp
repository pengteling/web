...
session("verifycode")=""
 ...
 rs.close
 set rs=nothing
 Response.Redirect "book_index2.asp?ok=ck"
 End IF
%>
...
<TABLE width=100% border=0 align=center cellPadding=3 cellSpacing=1 class="table_southidc">
  <FORM name=Form1 action=book_index2.asp method=post>
    <TBODY>
      <TR>
...
function ConfirmDel()
{
  ...
  <tr>
    <td bgcolor="#FFFFFF"><TABLE width=100% border=0 align=center cellPadding=5 cellSpacing=1 class="table_southidc">
      <TBODY>
        ...
        <TR>
          <TD colspan="2">栏目导航：<a href="book_index2.asp">所有留言</a> | <a href="book_index2.asp?tgsh=true">已审核留言</a> | <a href="book_index2.asp?tgsh=false">未审核留言</a>
          <br>
 ...
 shou  =Request("shou")
 tgsh=Request("tgsh")
 sql="select * from liuyan where B_type=0 "
 if shou<>"" then
...
<%If rs("tgsh") Then%>
<a href="book_index2.asp?work1=tgsh1&id=<%=rs("id")%>&work=yz"><font color="#000000">已经审核</font></a>
<%Else%>
<a href="book_index2.asp?work1=tgsh&id=<%=rs("id")%>&work=yz"><font color="#FF0000">通过审核</font></a>
<%End If%>
</TD>
        </TR>
