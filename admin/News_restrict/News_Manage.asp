...
Set rs= Server.CreateObject("ADODB.Recordset")
			...
			rs.open "select cateid,catename,followid,depth,sonid,parentid,orderID from category where catetype<=1 and cateid in ("&sonid_restrict&")  order by depth,orderID",conn,1,1
			if not rs.eof then
				data= rs.getRows()
			end if
			rs.close
...
if cateid<>0 then
	...
	sqlBigclass= "select cateid,catename from category where cateid in ("&parentid&") and cateid in ("&sonid_restrict&")  order by depth"
	sqlSmallclass="select cateid,catename from category where followid="&cateid&" and catetype<=1 and cateid in ("&sonid_restrict&")  order by orderID"
else
	
	sqlBigclass= "select  cateid,catename from category where 1=2 order by orderID"
	sqlSmallclass="select  cateid,catename from category where followid=0 and catetype<=1 and cateid in ("&sonid_restrict&")  order by orderID"
end if
...
sql="select news.*,c.catename from news left join category c on news.cateid=c.cateid where isdel=0"
if cateid<>0 then
...
sql=sql & " and news.cateid in ("&sonid_restrict&")  order by px desc,posttime desc,id desc"
  ...
  <tr> 
    <td width="100%" align="center" valign="top">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="table_southidc">
        <tr> 
          <td height="25">&nbsp;当前栏目路径： <a href=news_manage.asp>根栏目</a>  
          <%
...
do while not rsBigClass.eof
	if rsBigClass("cateid")=cateid then
		response.Write(" > <a href='News_Manage.asp?cateid=" & rsBigClass("cateid") & "'><font color='red'>" & rsBigClass("catename") & "</font></a>  ")		
	else
		response.Write(" > <a href='News_Manage.asp?cateid=" & rsBigClass("cateid") & "'>" & rsBigClass("catename") & "</a>")
	end if
	rsBigClass.movenext
loop
rsBigClass.close
	...
	if not (rsSmallClass.bof and rsSmallClass.eof) then
		...
		do while not rsSmallClass.eof
			if rsSmallClass("cateid")=cateid then
				response.Write("&nbsp;<a href='News_Manage.asp?cateid=" & rsSmallClass("cateid") & "'><font color='red'>" & rsSmallClass("catename") & "</font></a>&nbsp;&nbsp;")
				
			else
				response.Write("&nbsp;<a href='News_Manage.asp?cateid=" & rsSmallClass("cateid") & " '>" & rsSmallClass("catename") & "</a>&nbsp;&nbsp;")
			end if
...
%>
        <table class="table_southidc" border="0" cellspacing="1" width="100%" cellpadding="0" style="word-break:break-all">
          ...
          <tr class="tdbg"> 
            ...
            <td  bgcolor="#ECF5FF">
			<a href="../../ReadNews.asp?id=<%=rs("id")%>" target="_blank"><%=leftstr(rs("title"),68)%></a>
...
%>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="1">
          <tr> 
            ...
            <input type="radio" name="Action" value="Del"  checked>批量删除   <input type="radio" name="Action" value="move">移动到 <select name="cateid"> 
