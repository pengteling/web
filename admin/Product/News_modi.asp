...
Set NewsRs=Conn.execute("select * from product Where ID="&ID)
If NewsRs.Eof then
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="table_southidc">
  <form name="myform" method="post" action="News_save.asp" onSubmit="return CheckForm();">
    ...
    <tr bgcolor="#ECF5FF">
      ...
      <td colspan="2" width="80%">楼盘名称：<input name="title" type="text" class="input" size="10" value="<%=NewsRs("title")%>">
                
        项目地址：<input name="address" id="address" class="input" size="40" value="<%=NewsRs("address")%>"/>
        物业状态<select name="wystatus" >
        <option value="在售" <%if NewsRs("wystatus")="在售" then response.Write "selected"%>>在售</option>
        <option value="待售" <%if NewsRs("wystatus")="待售" then response.Write "selected"%>>待售</option>
        <option value="售馨" <%if NewsRs("wystatus")="售馨" then response.Write "selected"%>>售馨</option>
        </select>
    ...
    <tr bgcolor="#ECF5FF">
      ...
      <option value="高层住宅" <%if NewsRs("xmtype")="高层住宅" then response.Write "selected"%>>高层住宅</option>
      <option value="商业住宅" <%if NewsRs("xmtype")="商业住宅" then response.Write "selected"%>>商业住宅</option>
      <option value="花园洋房" <%if NewsRs("xmtype")="花园洋房" then response.Write "selected"%>>花园洋房</option>
      <option value="景观别墅" <%if NewsRs("xmtype")="景观别墅" then response.Write "selected"%>>景观别墅</option>
      </select>
        联系电话：<input name="tel" id="tel" class="input" size="10" value="<%=NewsRs("tel")%>"/>
        项目网址：<input name="xmurl" id="xmurl" class="input" size="40" value="<%=NewsRs("xmurl")%>"/>
...
%>    </select>   
        城市：<select name="City">
        <option value="重庆" <%if NewsRs("City")="重庆" then response.Write "selected"%>>重庆</option>
        <option value="成都" <%if NewsRs("City")="成都" then response.Write "selected"%>>成都</option>
 ...
 <tr bgcolor="#ECF5FF">
      ...
      <input type="checkbox" name="gzarea" value="30㎡-50㎡" <%if instr(NewsRs("gzarea"),"30㎡-50㎡")>0 then response.Write "checked"%>> 30㎡-50㎡   
       <input type="checkbox" name="gzarea" value="50㎡-80㎡" <%if instr(NewsRs("gzarea"),"50㎡-80㎡")>0 then response.Write "checked"%>> 50㎡-80㎡   
        <input type="checkbox" name="gzarea" value="80㎡-110㎡" <%if instr(NewsRs("gzarea"),"80㎡-110㎡")>0 then response.Write "checked"%>> 80㎡-110㎡   
         <input type="checkbox" name="gzarea" value="110㎡-150㎡" <%if instr(NewsRs("gzarea"),"110㎡-150㎡")>0 then response.Write "checked"%>> 110㎡-150㎡    
         <input type="checkbox" name="gzarea" value="150㎡以上" <%if instr(NewsRs("gzarea"),"150㎡以上")>0 then response.Write "checked"%>> 150㎡以上
            </td>
    </tr>
     ...
     <tr bgcolor="#ECF5FF">
      ...
      <input type="checkbox" name="roomsize" value="一居" <%if instr(NewsRs("roomsize"),"一居")>0 then response.Write "checked"%>>一居
       <input type="checkbox" name="roomsize" value="二居" <%if instr(NewsRs("roomsize"),"二居")>0 then response.Write "checked"%>>二居
       <input type="checkbox" name="roomsize" value="三居" <%if instr(NewsRs("roomsize"),"三居")>0 then response.Write "checked"%>>三居
       <input type="checkbox" name="roomsize" value="四居" <%if instr(NewsRs("roomsize"),"四居")>0 then response.Write "checked"%>>四居
       <input type="checkbox" name="roomsize" value="五居" <%if instr(NewsRs("roomsize"),"五居")>0 then response.Write "checked"%>>五居
       
       <input type="checkbox" name="roomsize" value="六居" <%if instr(NewsRs("roomsize"),"六居")>0 then response.Write "checked"%>>六居
       <input type="checkbox" name="roomsize" value="复式" <%if instr(NewsRs("roomsize"),"复式")>0 then response.Write "checked"%>>复式
       <input type="checkbox" name="roomsize" value="排屋" <%if instr(NewsRs("roomsize"),"排屋")>0 then response.Write "checked"%>>排屋
       <input type="checkbox" name="roomsize" value="跃层" <%if instr(NewsRs("roomsize"),"跃层")>0 then response.Write "checked"%>>跃层
       <input type="checkbox" name="roomsize" value="别墅" <%if instr(NewsRs("roomsize"),"别墅")>0 then response.Write "checked"%>>别墅
       <input type="checkbox" name="roomsize" value="别墅" <%if instr(NewsRs("roomsize"),"IH")>0 then response.Write "checked"%>>IH
            </td>
    </tr>
    <tr bgcolor="#ECF5FF">
      <td height="25" align="center" valign="top">项目简介：</td>
      <td colspan="2" valign="top"><textarea name="xmintro" id="xmintro" style="width:700px;height:100px;visibility:;"><%=NewsRs("xmintro")%></textarea>限255字</td>
    </tr>
