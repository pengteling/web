...
sql="select product.*,c.catename from product left join category c on product.cateid=c.cateid where isdel=0"
if cateid<>0 then
	sql=sql & " and product.cateid in (" & sonid & ")"	
end if
if keywords<>"" then
	sql=sql &" and (product.title like '%"&keywords&"%' or product.content like '%"&keywords&"%')"
end if
