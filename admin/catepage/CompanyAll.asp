...
If Act="del" Then
	...
	rs.Open"Select * From catepage Where cateID="&ID,Conn,1,3
	If Not rs.Eof Then
	...
	'response.write "delete from catepage where cateid in ("&sonid&")"
	conn.execute ("delete from catepage where cateid in ("&sonid&")")
	conn.execute ("Delete from news where cateid in ("&sonid&")")
	
	rs.open "select cateid,sonid from catepage where cateid in ("&parentid&") and cateid<>"&ID&" order by depth,orderID",conn,1,3
...
If Act="IsHide" Then
	...
	Conn.Execute("Update catepage Set IsHome="&s&" Where cateID="&ID&"")
	Response.Redirect("companyAll.asp")
	Response.End()
End If
If Act="IsOut" Then
	Conn.Execute("Update catepage Set IsOutLink=0 Where cateID="&ID&"")
	Response.Redirect("companyAll.asp")
	Response.End()
End If
If Act="IsPage" Then
	...
	If p=0 Then p=ID
	Conn.Execute("Update catepage Set IsIndex=0 Where cateID="&p&" Or followid="&p&"")
	Conn.Execute("Update catepage Set IsIndex=1 Where cateID="&ID&"")
	set Rs=server.createobject("adodb.recordset")
	Response.Redirect("companyAll.asp")
	Response.End()
End If
...
set rs=server.CreateObject("adodb.recordset")
			rs.open "select cateid,catename,followid,depth,sonid,parentid,orderID,cateType,isHome from catepage order by depth,orderID",conn,1,1
			if not rs.eof then
				data= rs.getRows()
			end if
			rs.close
...
if  data(3,i)=1  then '一级分类 和 有下级栏目的 分类显示 +号图标  or instr(data(4,i),",")>0
				...
				class_list=class_list&"<img src=""../Images/tree_folder"&imgno&".gif"" width=""19"" height=""18"" align=""absbottom""><b>"&data(1,i)&"</b> ("&data(6,i)&")</td>"
					...
					catetype= "栏目（列表）"
					 ...
					 case 0
					catetype= "栏目（内容）"
					 catetype_str="<a href=""companyadd.asp?Act=edit&ID="&data(0,i)&""">修改属性</a> | <a href=""company.asp?ID="&data(0,i)&""">修改内容</a> | <a href=""/admin/News/News_Manage.asp?cateid="&data(0,i)&""">内容管理</a> | <a href=""companyadd.asp?followid="&data(0,i)&""">添加下级栏目</a> | <a href=""companyAll.asp?Act=del&ID="&data(0,i)&""" onClick=""return confirm('删除之后,相关内容也会被删除,并且不能恢复!\n\n确定要删除吗？');"">删除</a>"
					case 2
					catetype= "单网页"
					catetype_str="<a href=""companyadd.asp?Act=edit&ID="&data(0,i)&""">修改属性</a> | <a href=""company.asp?ID="&data(0,i)&""">修改内容</a> | <a href=""companyAll.asp?Act=del&ID="&data(0,i)&""" onClick=""return confirm('删除之后,相关内容也会被删除,并且不能恢复!\n\n确定要删除吗？');"">删除</a>"
					case 3
					catetype= "单链接"
				catetype_str="<a href=""companyadd.asp?Act=edit&ID="&data(0,i)&""">修改属性</a> | <a href=""companyAll.asp?Act=del&ID="&data(0,i)&""" onClick=""return confirm('删除之后,相关内容也会被删除,并且不能恢复!\n\n确定要删除吗？');"">删除</a>"
					end select
