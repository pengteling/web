...
if upload.form("act")="uploadfile" then
 ...
 if (filelx<>"swf") and (filelx<>"jpg") and (filelx<>"jpgswf") then 
 ...
 if filelx="jpgswf" then
	if fileext<>"gif" and fileext<>"jpg" and fileext<>"swf" then
		response.write "<span style=""font-family: 宋体; font-size: 9pt"">只能上传jpg\gif\swf格式的文件！　[ <a href=# onclick=history.go(-1)>重新上传</a> ]</span>"
		response.end
     	end if
 end if
 if filelx="swf" then
 ...
 if filelx="jpgswf" then
	if file.filesize>(3000*1024) then
		response.write "<span style=""font-family: 宋体; font-size: 9pt"">最大只能上传 3M 的文件！　[ <a href=# onclick=history.go(-1)>重新上传</a> ]</span>"
		response.end
	end if
 end if
