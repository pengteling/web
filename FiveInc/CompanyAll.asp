...
sql = "select cateid,catename,followid,depth,sonid,parentid,orderID,cateType,isHome from category where 1=1 "&sql_sonid&" and modeltype<=1  order by depth,orderID"
...
$(function(){
	...
	$("tr[class^=td_southidc]").click(function(){
		...
		var isOpen=true;
		if ($(this).find("img").attr("src")==img1)
		{
		...
		var k=1;
		$(this).nextUntil("tr[class="+curClass+"]").each(function(index,element){
